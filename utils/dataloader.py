...
class ImageLoader_amb(VisionDataset):
    def __init__(self,
                 data_root,
                 image_size,
                 device='cuda:0',
        ...
        self.ae_name = ae_name
        self.device = device
        self.load_ambiant = load_ambiant
        self.process = VaeImageProcessor(do_convert_rgb=True)
        ...
        self.train_latent_save_path = os.path.join(self.data_path, f"train_latent_afhq")

        self.train_ambiant_save_path = os.path.join(self.data_path, f"train_ambiant_afhq")

        if os.path.exists(self.train_latent_save_path):
            self.data_train = torch.load(self.train_latent_save_path, weights_only=False)
            self.data_train_ambiant = torch.load(self.train_ambiant_save_path, weights_only=False)
        else:
            self.base_train_dataset = self.image_base_dataset(
                "train", self.latent_transform
            )
            self.data_train = self._get_latent(database=self.base_train_dataset,
                                               save_path_latent=self.train_latent_save_path,
                                               save_path_ambiant=self.train_ambiant_save_path,
                                               )
        # flt = torch.logical_or(self.data_train['label'] == 0, self.data_train['label'] == 1)
        # self.data_train["label"] = self.data_train["label"][flt]
        # self.data_train["latent"] = self.data_train["latent"][flt]
    ...
    def to_latent(self, x):
        with torch.no_grad():
            images = self.process.preprocess(x).to(x.device)
            outputs = self.vae.to(x.device).encode(images).latent_dist.mean
            outputs = (outputs - self.data_train['mean']) / self.data_train['std']
        return outputs
    ...
    def _get_latent(self, database, save_path_latent, save_path_ambiant):

        self.vae.to(self.device)
        data_loader = DataLoader(database, batch_size=512, shuffle=False)
        means, labels = [], []
        ...
        with torch.no_grad():
            for images, batch_labels in tqdm(data_loader, desc='Processing Dataset'):
                ...
                images = self.process.preprocess(images).to(self.device)
                # outputs = self.vae.to(self.device).encode(images).latent_dist.mean
                outputs = self.vae.encode(images).latent_dist.mean
                # if self.ae_name == 'stable_diff_14_aug_max' or self.ae_name == 'stable_diff_14_aug_max_max' or self.ae_name == 'stable_diff_14_aug_max_rot':
                #    outputs = outputs.reshape(sz[0], sz[1], 4, 16, 16)
                means.append(outputs.detach().cpu())
                labels.append(batch_labels)
                all_ambiant.append(images.cpu())

            latent_tensor = torch.cat(means, dim=0)
            label_tensor = torch.cat(labels, dim=0)

            all_ambiant = torch.cat(all_ambiant, dim=0)
            mean = latent_tensor.mean()
            std = latent_tensor.std()
            latent_tensor = (latent_tensor - mean) / std
            latent_dict = {"latent": latent_tensor,
                           ...
                           "mean": mean,
                           "std": std}
            ambiant_dict = {"ambiant": all_ambiant}

            torch.save(latent_dict, save_path_latent)
            torch.save(ambiant_dict, save_path_ambiant)
            print(f"Latent data saved at: {save_path_latent}")
            print(f"Ambiant data saved at: {save_path_ambiant}")
