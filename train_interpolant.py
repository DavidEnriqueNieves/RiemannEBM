...
def main(args):
    with torch.no_grad():
        ...
        if args.dataset == 'afhq':
            ...
            #                                   shuffle=True,
            #                                   drop_last=True)
            ## load the encoding network
            # ae_path = "/media/data_cifs/projects/prj_mental/model/AE/alphanum/AE_24012025_w_kl=0_MLP_w_trip1//last.model"
            # ae_param = torch.load(ae_path, weights_only=False)
            # ae = ae_param["type"](**ae_param["args"]).to(args.device)
            # stats = ae_param["stats"]
            # ae.load_state_dict(ae_param["weight"])
            # ae.eval()
            ...
            # if args.metric == "conf_rbf_invp":
            #    im_cat = d_loader.data_train_ambiant["ambiant"][d_loader.data_train['label'] == 0]
            #    im_dog = d_loader.data_train_ambiant["ambiant"][d_loader.data_train['label'] == 1]
            ...
            # dset_z = x_to_z(dset.view(-1, 1, 32, 32))
            # dset_z = dset_z.view(7, 180, 64)
            # dset_z = dset_z.view(8, 180, 64)
            # z_test_0 = dset_z[:, 50].unsqueeze(1)
            # z_test_1 = dset_z[:, -45].unsqueeze(1)
            # z_test_0 = z_test_0.repeat(1, args.t_steps, 1).to(args.device).detach()
            # z_test_1 = z_test_1.repeat(1, args.t_steps, 1).to(args.device).detach()
            # t_test = torch.linspace(0, 1, args.t_steps).unsqueeze(0).unsqueeze(-1).to(args.device).detach()
            # t_test = t_test.expand_as(z_test_0).detach()
        ...
        if approximator == 'ebm':
            ...
            if h_function == "invp":
                ...
                # random.seed(1)
                """
                idx_image_cat_d = torch.randint(low=0, high=lt_cat.size(0), size=(50,))
                idx_image_dog_d = torch.randint(low=0, high=lt_dog.size(0), size=(50,))
                z_d_0 = lt_cat[idx_image_cat_d].unsqueeze(1).to(args.device)
                z_d_1 = lt_dog[idx_image_dog_d].unsqueeze(1).to(args.device)

                z_d_0 = z_d_0.repeat(1, args.t_steps, *[1 for _ in lt_size])
                z_d_1 = z_d_1.repeat(1, args.t_steps, *[1 for _ in lt_size])
                t_d = torch.linspace(0, 1, args.t_steps).view(1, args.t_steps, *[1 for _ in lt_size]).to(
                    args.device).detach()
                t_d = t_d.repeat(z_d_0.size(0), 1, *[1 for _ in lt_size])
                z_t_d = (1 - t_d) * z_d_0 + t_d * z_d_1
                #te = h(z_t_d)
                #a=1
                #z0 = dset_z[:, -45].view(-1, *lt_size).unsqueeze(1)
                #z1 = dset_z[:, 45].view(-1, *lt_size).unsqueeze(1)
                #t = torch.linspace(0, 1, args.t_steps).unsqueeze(0).unsqueeze(-1).to(args.device).detach()
                #normalizing_set = (1-t)*z0 + t*z1
                args_normalization = {"min": args.min_h, "max": args.max_h}

                #h.global_normalization(normalizing_set, **args_normalization)
                """
