...
#from models.VanillaNet_Large import VanillaNet_L, VanillaNet_Indiana_Jones
#from models.ResNet import Res18_Quadratic, MegaMLP_Quadratic, GigaAttentionRes18
#from models.FixUpResNet import fixup_resnet18, fixup_resnet50
...
def main(args):
    ...
    if args.training == "cd" or args.training == 'cdsm':
        ...
        #optimizerE = torch.optim.Adam(netE.parameters(), lr=args.lr_init)

        #optimizerE = torch.optim.Adam(netE.parameters(), lr=args.lr_init)
    optimizerE = torch.optim.AdamW(netE.parameters(), lr=args.lr_init, weight_decay=1e-5)
    if args.gamma_scheduler != 0:
            #scheduler = StepLR(optimizerE, step_size=100, gamma=0.5)
            #scheduler = MultiStepLR(optimizerE, milestones=args.step_scheduler, gamma=args.gamma_scheduler)
            #scheduler = MultiStepLR(optimizerE, milestones=[500, 1000], gamma=0.5)
    ...
    if args.training == "dsm" or args.training == 'cdsm':
        ...
        #sigmas_dsm = torch.linspace(0.05, 1.2, args.batch_size).view((args.batch_size, 1, 1, 1)).to(args.device)
        #sigma0 = 0.1
        #sigma02 = sigma0 ** 2
    ...
    for ep in range(args.epoch):
        ...
        for idx_batch, (latent, label) in enumerate(pbar):
            ...
            if args.training == 'dsm' or args.training == 'cdsm':
                ...
                #latent_noisy = latent + sigmas_dsm*torch.randn_like(latent)
                ...
                #dsm_loss = ((((latent - latent_noisy) / sigmas_dsm / sigma02 + grad_x / sigmas_dsm) ** 2) / len(latent_noisy)).sum()
                sig = (1-baralphas[timesteps].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) + 1e-3)
                dsm_loss_2 = ((1/sig) * ((latent_noisy_m-latent_noisy) + grad_x)**2).sum()/len(latent)
            ...
            if args.training == 'cd' or args.training == 'cdsm':
                ...
                #    z_corrupt = latent + args.noise_grad_regul*torch.randn_like(latent)
                #    z_corrupt.requires_grad_()
                #    energy_corrupt = netE(z_corrupt)
                #    f_prime_corrupt = torch.autograd.grad(
                #        outputs=energy_corrupt,
                #        inputs=z_corrupt,
                #        grad_outputs=torch.ones_like(energy_corrupt),  # same shape as f
                #        create_graph=True,
                #        retain_graph=True
                #    )[0]
                    # Tfel:  z_denoised = z_corrupt - args.sgld_lr * torch.exp(multpilier) * f_prime_corrupt
                #    z_denoised = z_corrupt - args.sgld_lr * f_prime_corrupt

                #    grad_regul = (z_denoised - latent).pow(2).mean()
                    ## Tfel : Cosine + L2
                #else:
                #    grad_regul = torch.tensor(0.0)

                #if args.w_last_gradient != 0:
                #    netE.requires_grad_(False)
                #    loss_last_grad = netE(z_lst_grad)
                #    netE.requires_grad_(True)
                #else:
                #    loss_last_grad = torch.tensor(0.0)
                #l_p_x = loss_last_grad
                #l_p_x = (fp - fq) + args.w_regul*reg_low_en + args.grad_regul*grad_regul + args.w_last_gradient*loss_last_grad.mean()
            ...
            if torch.isnan(max_grad).any():
                print('update')
            #if is_nan or max_grad > 1000:
            #    print(f'ignore optim at step {idx_batch}')
            #    continue
            optimizerE.step()
        ...
        if ep % 10 == 0:
            netE.eval()
            #initial_x = torch.randn_like(latent).to(args.device)
            #z_q, _ = Annealed_Langevin_E(netE, initial_x, 0.02, T)
            #z_q, _ = Annealed_Langevin_E(netE, initial_x, 0.02, T)
            #z_q, norm_gr = sample_q(netE, replay_buffer)
            #if args.training == 'cd':
            #if len(replay_buffer) > 0:
                #if args.energy_func == "latent_conv":
            rdn_buff = init_random((0, 4, 16, 16), init_type=args.init_type)
                #else:
                #    rdn_buff = init_random((0, ae_param["args"]["z_dim"]), init_type=args.init_type)
            z_q, norm_gr, _, _ = sample_q(netE, rdn_buff, n_steps=1000, clip=args.gradient_clip)
                #else:
                #    z_q, norm_gr, _ = sample_q(netE, replay_buffer)
            #elif args.training == 'dsm':
            #    init = init_random((100, ae_param["args"]["z_dim"]), init_type=args.init_type)
            #    init = init.to(args.device)
            #    z_q, E_trace = Annealed_Langevin_E(netE, init, 0.02, T, 100, cond=None)
            #x_q = d_loader.to_im(z_q.cpu().detach())
            z_q = z_q*2
            x_q = d_loader.to_im(z_q.detach())
            #z_q = normalize_1(z_q, stats, unormalize=True)
            #x_q = ae.decode(z_q)
            if args.model_name is not None:
                to_plot = wandb.Image(
            ...
            #dico_log = {"train/loss": l_p_x.item(),
            #        "train/E_data": fp.item(),
            #        "train/E_sampled": fq.item(),
            #        "train/lr": optimizerE.param_groups[0]['lr'],
            #        "train/norm_grad": norm_gr.item()
            #        "train/regul_low_en": reg_low_en.item()
            #        }
            #wandb.log(dico_log)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr_init', type=float, default=1e-4, help='init lr of the optimizer')
    ...
    parser.add_argument('--sgld_std', type=float, default=5e-3, help='std in the mcmc sampling')
    ...
    parser.add_argument("--multiplier", type=int, default=1, help='channel multiplier of the network')

    #parser.add_argument('--base_path_ae', type=str, default='/media/data_cifs/projects/prj_mental/model/AE/alphanum/',
    #                    help='based path of the ae')

    #parser.add_argument('--ae', type=str, default=None,help='path of the ae')
    ...
    ## For contrastive divergence only




    #
    #parser.add_argument('--lr_min', type=float, default=1e-2, help='min lr of the optimizer')
    #parser.add_argument('--lr_decay', type=float, default=1.0, help='decay of the optimizer learning rate')


    #parser.add_argument('--energy_func', type=str, default='simple_mlp', choices=['simple_mlp', 'l2_norm', 'latent_conv'])
    #
